...
def test_ai_parser_is_strict_and_bounded():
    ...
    with pytest.raises(ValueError, match="invalid_categories"):
        parse_output('{"summary":"ok","categories":["unknown"],"relevant_company_ids":[]}', set())
    with pytest.raises(ValueError, match="invalid_output_schema"):
...
def test_ai_hash_excludes_model_and_prompt_configuration():
    ...
    prompt = json.loads(build_prompt(row))
    assert "instruction" in prompt
    assert "use other when no listed category fits" in prompt["instruction"]